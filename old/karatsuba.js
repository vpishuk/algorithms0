function karatsuba(num1, num2, step = 0) {
  ...
  if (num1.length <= 1 || num2.length <= 1) {
    const res = (parseInt(num1 || 0, 10) * parseInt(num2 || 0, 10)).toString();
...
function sumStrings(num1, num2) {
  ...
  for (let i = up.length - 1, k = down.length - 1; i >= 0; i--, k--) {
    const sum = parseInt(up[i] || 0, 10) + parseInt(down[k] || 0, 10) + increment;
    if (sum >= 10 && i > 0) {
...
function minusStrings(num1, num2) {
  ...
  for (let i = num1.length - 1, k = num2.length - 1; i >= 0; i--, k--) {
    ...
    const downDigit = parseInt(num2[k] || 0, 10);
    if (upDigit === 0 && i === 0) {
      break;
    }
    ...
    const sum = Math.abs(upDigit - downDigit);
    res = sum.toString() + res;
  }
  return res;
}
